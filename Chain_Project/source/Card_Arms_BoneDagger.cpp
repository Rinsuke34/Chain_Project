...
// コンストラクタ
Card_Arms_BoneDagger::Card_Arms_BoneDagger() : Card_Arms_Base()
{
	/* カード情報の設定 */
	// レアリティ
	...
	// 画像の名前
	this->ImageName = "BoneDagger";		// 画像の名前
	// 説明文
...
// 効果発動:ターン開始時
void Card_Arms_BoneDagger::Effect_StartTurn()
{
	/* 捨てられたカード枚数を取得 */
	int Discarded_Card_Count = this->pDataList_Battle->GetTrashCardList().size();

	/* 攻撃力を所持しているコイン数にする */
	this->Strength = 8 + Discarded_Card_Count;
