...
/* 使用する要素のインクルード */
// ヘッダファイル
#include "Character_Npc_Goblin.h"
// 関連クラス
...
// コンストラクタ
Character_Npc_Goblin::Character_Npc_Goblin() : Character_Base()
{
	/* 初期化 */
	...
	SetUpImage("Character_Ilust/Npc/Goblin");
...
// 行動
void Character_Npc_Goblin::Action()
{
	/* ランダムに行動を行う */
		...
		case 0:
			{
				...
				addEffect->ExplanationText	= "のうりょくきょうか";	// 説明文
...
void Character_Npc_Goblin::Action_Extra()
{
	/* 攻撃バフを味方全員にかける */
	// 前衛から順に敵が存在するか確認
	for (int i = 0; i < DataList_Battle::POSITION_MAX; i++)
	{
		std::shared_ptr<Character_Base> Enemy = this->pDataList_Battle->GetEnemyCharacter(i);
		if (Enemy != nullptr)
		{
			// 敵が存在するなら
			/* 対象が"状態異常：攻撃力強化"を所持しているか確認 */
			std::shared_ptr<Character_Buff_Debuff_Base> Strength = Enemy->CheckGet_Buff_Debuff("Buff_Strength");
			if (Strength != nullptr)
			{
				// 所持しているなら
				/* 残りターン数を+5する */
			...
			/* 対象が"状態異常：防御力強化"を所持しているか確認 */
			std::shared_ptr<Character_Buff_Debuff_Base> Defence = Enemy->CheckGet_Buff_Debuff("Buff_Diffence");
			if (Defence != nullptr)
			{
				// 所持しているなら
				/* 残りターン数を+3する */
