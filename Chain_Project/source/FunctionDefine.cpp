...
namespace DRAW_FUNCTION
{
	...
	// 鎖の描写（アニメ付き）
	void DrawChain_Anim(Struct_2D::POSITION StartPos, Struct_2D::POSITION EndPos, int AnimCount)
	{
		// 引数
		// StartPos		<- 鎖の開始位置
		// EndPos		<- 鎖の終了位置
		// AnimCount	<- アニメーションカウント(0～40)(※この値分開始位置がずれる)

		// AnimCount をクランプ
		if (AnimCount < 0) AnimCount = 0;
		if (AnimCount > 40) AnimCount = 40;
		...
		// 基本の中央寄せオフセット（DrawChain と同じ）
		double baseOffset = step * 0.5;

		// AnimCount に応じた追加シフト（0..step）
		double animShift = (static_cast<double>(AnimCount) / 40.0) * step;

		// 最終オフセット（AnimCount==0 のとき baseOffset と同じ）
		double offset = baseOffset + animShift;
	...
	// 道の描写
	void DrawRoad(Struct_2D::POSITION StartPos, Struct_2D::POSITION EndPos)
	{
		// 引数
		// StartPos	<- 道の開始位置
		// EndPos	<- 道の終了位置
		...
		/* 道の画像を取得 */
		std::string ImageFilePath = "WoldMap_Node/Road";
		std::shared_ptr<int> Image = pDataList_Image->iGetImageHandle(ImageFilePath);
		if (!Image || *Image < 0) return;
	...
	void DrawFrame_Image(Struct_2D::POSITION CenterPos, Struct_2D::POSITION IconSize, int Frame_Thickness, int Image_Corner, int Image_Line, int Image_Inside)
	{
		...
		// ※画像ハンドルの代わりに-1が代入されたら、その部分の描写を行わない
