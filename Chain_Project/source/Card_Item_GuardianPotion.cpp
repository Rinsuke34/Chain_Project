...
Card_Item_GuardianPotion::Card_Item_GuardianPotion() : Card_Item_Base()
{
	...
	// 説明文
	this->ExplanationText = "じぶんにディフェンスアップをXをふよ/nXはチェイン+１";
}
...
void Card_Item_GuardianPotion::Card_Effect_Extra_Process()
{
	...
	if (GuardianBuff != nullptr)
	{
		// 所持しているなら
		/* 残りターン数をチェイン数分加算する */
		GuardianBuff->Buff_Debuff_Time += GetNowChainCount() + 1;	// 残りターン数 += チェイン数 + 1
	}
	else
	{
		// 所持していないなら
		...
		New_Guardian_Buff->Buff_Debuff_Time	= GetNowChainCount() + 1;	// 残りターン数 = チェイン数 + 1
